...
export function sortConversations(
  ...
  const time1 = conversation1Timestamp ? new Date(conversation1Timestamp).getTime() : Infinity;
  const time2 = conversation2Timestamp ? new Date(conversation2Timestamp).getTime() : Infinity;
